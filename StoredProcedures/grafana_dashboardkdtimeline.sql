...
begin 
	...
	select date_add(date(matchTime), interval -3 hour) as `Date`, fn_calculateKD(sum(kills), sum(deaths)) as `Kill\Deaths`, fn_calculateKD(sum(revives), sum(deaths)) as `Revive\Deaths`
	from getstatsbytime 
