...
--	call sp_getstatsbytime(fn_getdatetimefromrange(lastVar,lastTypeVar,now()),now());
	...
	select playerName as Nickname, steamId as SteamID, sum(revives) as Revives, sum(wounds) as Wounds, sum(kills) as Kills, sum(deaths) as Deaths, sum(teamkills) as Teamkills, sum(vehicles) as `Destroyed Vehicles`, fn_calculateKD(sum(kills), sum(deaths)) as `Kill\Deaths`, fn_calculateKD(sum(revives), sum(deaths)) as `Revive\Deaths`
	from getstatsbytime 
	...
	order by (sum(vehicles)*5) + sum(revives) + sum(kills) + ((sum(wounds) - sum(kills))/10) - (sum(deaths) - sum(teamkills)) desc;
