...
begin
	...
	insert into getstatsbytime (steamID, teamkills, serverID, matchID)
	select attacker as steamID, count(victim) as teamkills, server as serverID, `match` as matchID
	from dblog_wounds
	where teamkill = 1 and attacker != "00000000000000000" and attacker != victim
		and `server` is not null and `match` is not null and attacker is not null and attacker not in (select a.steamId from dblog_players as a where a.ignore = 1)
		and time between startDate and endDate
	group by server, `match`, attacker
	on duplicate key update teamkills = values(teamkills);
